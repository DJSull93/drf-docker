...
from project2.common.models import FileDTO, Printer, Reader
import folium
'''
문제 정의 : 서울시의 범죄현황과 cctv 현황을 분석하여 정해진 
예산 안에서 구별로 다음해 배분 기준안 제시.

예산 금액 입력 -> 구 별 할당 CCTV 카운터를 자동으로 알려주는
AI 프로그램을 작성하시오.
'''
...
class CrimeService(Reader):
    ...
    def __init__(self):
        ...
        self.arrest_rate_columns = ['살인검거율', '강도검거율', '강간검거율', '절도검거율', '폭력검거율']
    ...
    def save_police_pos(self):
        ...
        for name in station_names:
            ...
            station_adress.append(t[0].get('formatted_address'))
    ...
    def save_police_norm(self):
        ...
        for i in self.arrest_rate_columns:
        ...
        x = police[self.arrest_rate_columns].values
        ...
        police_norm[self.arrest_rate_columns] = police[self.arrest_rate_columns]
        police_norm['범죄'] = np.sum(police_norm[self.arrest_rate_columns], axis=1)
        police_norm['검거'] = np.sum(police_norm[self.crime_columns], axis=1)
        police_norm.to_csv('./saved_data/police_norm.csv', sep=',', encoding='UTF-8')

    def save_folium_map(self):
        # police_norm.csv, geo_simple.json, crime_cctv.csv, police_pos.csv
        ...
        fm = folium.Map(location=[37.5502, 126.982], zoom_start=12, title='Stamen Toner')
        ...
        for i in police_pos.index:
            folium.CircleMarker([police_pos['lat'][i], police_pos['lng'][i]],
                            ...
                            fill_color='#0a0a32').add_to(fm)

        folium.LayerControl().add_to(fm)

        fm.save('./saved_data/seoul_crime.html')
