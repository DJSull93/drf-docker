...
class AdminTests(TestCase):
    ...
    def setUp(self):
        self.admin = User.objects.create_superuser('mywork', 'mywork@example.com',
                                                   'mywork')
    ...
    def _login_superuser(self):
        self.client.login(username='mywork', password='mywork')

    def test_view_manage_wrong_obj(self):
        self._login_superuser()
        url = reverse('mywork:%s_%s_permissions_manage_user' % self.obj_info,
    ...
    def test_view(self):
        self._login_superuser()
        url = reverse('mywork:%s_%s_permissions' % self.obj_info,
                      args=[self.obj.pk])
        response = self.client.get(url)
    ...
    def test_view_manage_user_form(self):
        ...
        redirect_url = reverse('mywork:%s_%s_permissions_manage_user' %
    ...
    def test_view_manage_group_form(self):
        ...
        self.assertEqual(len(response.redirect_chain), 1)
        self.assertEqual(response.redirect_chain[0][1], 302)
        redirect_url = reverse('mywork:%s_%s_permissions_manage_group' %
    ...
    def test_view_manage_group_wrong_perms(self):
        self._login_superuser()
        url = reverse('mywork:%s_%s_permissions_manage_group' %
                      self.obj_info, args=[self.obj.pk, self.group.id])
        perms = ['change_user']  # This is not self.obj related permission
        data = {'permissions': perms}
        response = self.client.post(url, data, follow=True)
...
if 'django.contrib.mywork' not in settings.INSTALLED_APPS:
    # Skip mywork tests if mywork app is not registered
    # we simpy clean up AdminTests class ...
    # TODO: use @unittest.skipUnless('django.contrib.mywork' in settings.INSTALLED_APPS)
